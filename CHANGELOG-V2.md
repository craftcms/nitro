...
- Added the `nitro craft` command to run `craft` console commands in a site’s container.
...
- Added the `nitro queue` command, which allows you to listen to Craft’s queue.
...
- Most Nitro commands are now context aware of the directory they are executed in.
